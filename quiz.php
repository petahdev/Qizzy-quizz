...
<body>
    <h1>Welcome to the Personalized Quiz!</h1>
    ...
    if (!isset($_SESSION['questions']) || isset($_POST['reset'])) {
        ...
        $_SESSION['questions'] = array_merge(array_slice($easyQuestions, 0, 7), array_slice($hardQuestions, 0, 3));
    ...
    if ($quizCompleted) {
        ...
        if ($score >= 9) {
            $profit_amount = $stake_amount * 1.9; // 100% profit + stake back
        } elseif ($score == 8) {
            $profit_amount = $stake_amount * 1.5; // 50% profit + stake back
        } else {
